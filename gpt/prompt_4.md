  ...
  void _submit() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        final result = await ApiService().coachAddTrainingSession(
          authToken,
          startTime,
          endTime,
          selectedCoachId,
        ...
        final sessionId = result['session']['session_id'];

        // Register coach participation as 'Yes'
        await ApiService().studentRegisterParticipation(
          authToken,
          sessionId,
          'Yes',
        );
...
class CoachTabState extends State<CoachTab> {
  ...
  Widget _buildSessionRow(Map<String, dynamic> session) {
    ...
    return ListTile(
      ...
      trailing: session['state'] == 'archived'
          ? null
          : isRegisterable
              ? ElevatedButton(
