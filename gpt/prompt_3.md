...
class CoachTabState extends State<CoachTab> {
  ...
  void _cancelSession(String sessionId) async {
    try {
      ...
      final index = _sessions.indexWhere((session) => session['session_id'] == sessionId);
      if (index != -1) {
        setState(() {
          _sessions[index]['state'] = "cancelled";
        });
      }
    } catch (e) {
      _showError(e.toString());
  ...
  Widget _buildSessionRow(Map<String, dynamic> session) {
    ...
    final isCancelable = startTime.difference(now).inMinutes > 10;
    final isRegisterable = now.isAfter(startTime.subtract(Duration(minutes: 10))) &&
  ...
  void _submit() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        _authToken = result['user']['auth_token'];
