...
class AdminTabState extends State<AdminTab> {
  ...
  void _createUser() async {
    ...
    if (result != null && result == true) {
      ...
      setState(() {
        // Re-fetch user data or update _users accordingly
      });
    }
  }
  ...
  Future<String?> _retrieveAuthToken(String userId) async {
    try {
      final result = await ApiService().adminShowAuthToken(_authToken, userId);
  ...
  Widget build(BuildContext context) {
    // Sort users by name
    final sortedUsers = _users.entries.toList()
...
class CoachTabState extends State<CoachTab> {
  ...
  void _refreshData() async {
    try {
      ...
      if (_sessions.isNotEmpty) {
        final firstSessionId = _sessions.first['session_id'];
  ...
  void _updateSessionState(
    ...
    try {
      await ApiService()
          .coachUpdateTrainingSessionState(_authToken, sessionId, newState);
      if (newState == 'deleted') {
        setState(() {
  ...
  void _viewParticipantHistory() async {
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => ParticipationHistoryScreen(
          authToken: _authToken,
          users: _users,
        ),
      ),
              ...
              Focus(
                ...
                child: DropdownButton<String>(
                  ...
                  items: _getAllowedStates(session['state'])
                    ...
                    value: state,
                    child:
                    Text(state[0].toUpperCase() + state.substring(1)),
                  ))
                      .toList(),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildParticipationDetails(session),
              ],
            ),
          ),
  ...
  Widget _buildParticipationDetails(Map<String, dynamic> session) {
...
class ParticipationTabState extends State<ParticipationTab> {
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8.0),
                _buildParticipationDetails(sessionId),
              ],
            ),
          ),
        if (isExpanded) const Divider(), // Only show divider after expanded session
      ],
    );
  }
  ...
  String _formatTimeRange(DateTime startTime, int endTimeEpoch) {
    final endTime =
        DateTime.fromMillisecondsSinceEpoch(endTimeEpoch * 1000);
    return '${_padZero(startTime.hour)}:${_padZero(startTime.minute)} - ${_padZero(endTime.hour)}:${_padZero(endTime.minute)}';
  }
...
class ParticipationHistoryScreenState extends State<ParticipationHistoryScreen> {
  late DateTime _startDate;
  late DateTime _endDate;
  final List<bool> _weekDaysSelected = List<bool>.filled(7, true); // All weekdays selected
  ...
  void initState() {
    super.initState();
    // Initialize _startDate and _endDate to 00:00 of their respective days
    final now = DateTime.now();
  ...
  void _pickStartDate() async {
    final date = await showDatePicker(
      ...
      lastDate: _endDate, // Ensure start date is not after end date
    );
    if (date != null) {
      setState(() {
        _startDate = DateTime(date.year, date.month, date.day);
        // Reprocess data if history data is available
  ...
  void _pickEndDate() async {
    final date = await showDatePicker(
      ...
      firstDate: _startDate, // Ensure end date is not before start date
  ...
  void _submit() async {
    ...
    try {
      ...
      int endTime = (_endDate.add(const Duration(days: 1))).millisecondsSinceEpoch ~/ 1000; // Include entire end date
  ...
  void _processData(Map<String, dynamic> data) {
    ...
    _participantIds.sort((a, b) => (widget.users[a]?['name'] ?? '').compareTo(widget.users[b]?['name'] ?? ''));
  ...
  String _formatDateOnly(DateTime dateTime) {
    // Ensure consistent length by using fixed-width formatting
  ...
  String _getWeekday(int weekday) {
    // Using fixed-length abbreviations
          ...
          children: [
            Row(
              children: [
                Expanded(
                  child: ListTile(
                    title: Text('Start Date: ${_startDate.year}-${_padZero(_startDate.month)}-${_padZero(_startDate.day)}'),
                    trailing: const Icon(Icons.calendar_today),
                ...
                Expanded(
                  child: ListTile(
                    title: Text('End Date: ${_endDate.year}-${_padZero(_endDate.month)}-${_padZero(_endDate.day)}'),
                    trailing: const Icon(Icons.calendar_today),
                    onTap: _pickEndDate,
                  ),
                ),
  ...
  Widget _buildParticipationMatrix() {
    ...
    for (String participantId in _participantIds) {
      String name = widget.users[participantId]?['name'] ?? 'Unknown';
      headerCells.add(
        Container(
...
class UpdateTrainingSessionScreenState extends State<UpdateTrainingSessionScreen> {
  final _formKey = GlobalKey<FormState>();
...
class UpdateUserScreenState extends State<UpdateUserScreen> {
  ...
  void initState() {
    super.initState();
    final user = widget.users[widget.userId];
    _name = user['name'];
    _role = user['role'];
  }
  ...
  void _deleteUser() async {
    ...
    if (confirm == true) {
      try {
        await ApiService().adminDeleteUser(widget.authToken, widget.userId);
        if (mounted) Navigator.pop(context, true);
      } catch (e) {
        _showError(e.toString());
      }
