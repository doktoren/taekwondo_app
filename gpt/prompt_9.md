...
class CoachTabState extends State<CoachTab> {
  ...
  void _updateSessionState(
    ...
    try {
      final dataProvider = Provider.of<DataProvider>(context, listen: false);
      await ApiService()
          .coachUpdateTrainingSessionState(dataProvider.authToken, sessionId, newState);
  ...
  Widget build(BuildContext context) {
    ...
    final dataProvider = Provider.of<DataProvider>(context);
    final sessionsMap = dataProvider.userData['sessions'] as Map<String, dynamic>? ?? {};
    ...
    sessions.sort((a, b) => (a['start_time'] as int).compareTo(b['start_time'] as int));
    ...
    // **Add this logic to expand the first session by default**
    if (filteredSessions.isNotEmpty && _expandedSessions.isEmpty) {
...
class ParticipationTabState extends State<ParticipationTab> {
  ...
  void _updateParticipation(String sessionId, String? joining, Map<String, dynamic> sessions) async {
    String? apiJoining =
            ...
            children: [
              DropdownButton<String>(
                ...
                onChanged: (session['state'] != 'scheduled')
                    ...
                    : (value) {
                  if (value != null) {
                    _updateParticipation(sessionId, value, {sessionId: session});
                  }
                },
              ),
              IconButton(
  ...
  Widget build(BuildContext context) {
    ...
    if (activeSessions.isNotEmpty && _expandedSessions.isEmpty) {
