...
class AdminTabState extends State<AdminTab> {
  ...
  void initState() {
    super.initState();
    final dataProvider = Provider.of<DataProvider>(context, listen: false);
    _authToken = dataProvider.authToken;
    _users = dataProvider.userData['users'];
  }
...
class CoachTabState extends State<CoachTab> {
  ...
  void initState() {
    super.initState();
    final dataProvider = Provider.of<DataProvider>(context, listen: false);
    _sessions = dataProvider.userData['sessions'].values.toList();
    _users = dataProvider.userData['users'];
...
class ParticipationTabState extends State<ParticipationTab> {
  ...
  void initState() {
    super.initState();
    final dataProvider = Provider.of<DataProvider>(context, listen: false);
    _sessions = dataProvider.userData['sessions'] as Map<String, dynamic>;
    _users = dataProvider.userData['users'];
    _userId = dataProvider.userId;
    _loadActingForUsers();
  }
  ...
  void _refreshData() async {
    try {
      ...
      await dataProvider.refreshData();
      setState(() {
        _sessions = dataProvider.userData['sessions'];
        _users = dataProvider.userData['users'];
      });
    } catch (e) {
...
class ParticipationHistoryScreenState
  ...
  void _processData(Map<String, dynamic> data) {
    ...
    _participantIds.sort((a, b) => (_users[a]?['name'] ?? '').compareTo(_users[b]?['name'] ?? ''));
  ...
  Widget _buildParticipationMatrix() {
    ...
    for (String participantId in _participantIds) {
      String name = _users[participantId]?['name'] ?? 'Unknown';
      headerCells.add(
        Container(
