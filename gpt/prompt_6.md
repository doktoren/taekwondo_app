...
class CoachTabState extends State<CoachTab> {
  ...
  void _updateSessionState(String sessionId, String newState, Map<String, dynamic> session) async {
    try {
      await ApiService().coachUpdateTrainingSessionState(_authToken, sessionId, newState);
      if (newState == 'deleted') {
        setState(() {
  ...
  Widget _buildSessionRow(Map<String, dynamic> session) {
    ...
    final startTime = DateTime.fromMillisecondsSinceEpoch(session['start_time'] * 1000);
    final endTime = DateTime.fromMillisecondsSinceEpoch(session['end_time'] * 1000);
